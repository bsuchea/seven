...
<body class="app">
    ...
    <?php
        ...
        // get sale data
        $saleReportData = $con->prepare("SELECT * FROM tbl_purchase INNER JOIN tbl_suppliers ON
                                        tbl_purchase.suppliers_id = tbl_suppliers.suppliers_id WHERE purchase_id = ?");
        $saleReportData->bindParam(1, $_GET['id']);
        $saleReportData->execute();
        $sale_report_data = $saleReportData->fetch(PDO::FETCH_BOTH);

        // get sale detail
        $saleDetail = $con->prepare("SELECT * FROM tbl_purchase_detail
            INNER JOIN tbl_item ON tbl_purchase_detail.item_id = tbl_item.item_id
            WHERE tbl_purchase_detail.purchase_id = ? ORDER BY item_name ASC");
                ...
                <div class="col-12 col-lg-12">
                    ...
                    <div class="row">
                        ...
                        <div class="mt-1 mb-1 text-secondary-m1 text-600 text-125" style="font-family: 'Kantumruy Pro ExtraLight';">
                                    <h5><strong>ព័ត៌មានអ្នកផ្គត់ផ្គង់</strong></h5>
                            ...
                            <div>
                                ...
                                <span class="text-sm text-grey-m2 align-middle" style="font-family: 'Kantumruy Pro ExtraLight';"><strong>ឈ្មោះអ្នកផ្គង់ផ្គង់ : </strong></span>
                                <span class="text-600 text-110 text-blue align-middle"><?= $sale_report_data['suppliers_name'] ?></span>
                            </div>
                            <div class="text-grey-m2">
                                ...
                                </svg><span style="font-family:'Kantumruy Pro ExtraLight';"><strong> លេខអ្នកផ្គត់ផ្គង់ : </strong></span> <?= $sale_report_data['suppliers_phone']?></div>
                            </div>
                        </div>
                        ...
                        <div class="text-95 col-sm-6 align-self-start d-sm-flex justify-content-end">
                            ...
                            <div class="text-grey-m2">
                                ...
                                <span class="text-600 text-90"><strong>កាលបរិច្ឆេទ : </span> <?= $sale_report_data['purchase_date'] ?></strong></div>
                        ...
                        <div class="text-95 text-secondary-d3">
                            ...
                            <?php foreach($saleDetail as $items): ?>
                                <div class="row mb-2 mb-sm-0 py-25 p-3 border border-info rounded-3">
                                    ...
                                    <div class="d-none d-sm-block col-2"><?= $items['purchase_qty'] ?></div>
                                    <div class="d-none d-sm-block col-2 text-95">$<?= $items['unit_price'] ?></div>
                                    <div class="col-2 text-secondary-d2">$<?= $items['purchase_qty'] * $items['unit_price'] ?></div>
                                </div>
                            <?php 
                                $totalQTY += $items['purchase_qty'];
                                $totalAmount += $items['purchase_qty'] * $items['unit_price'];
                            endforeach;
                            ?>
                        </div>
