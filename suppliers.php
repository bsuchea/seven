                ...
                <div class="row g-2 justify-content-between mb-1">
                    <div class="col-auto">
                        <h1 style="font-family: 'Kantumruy Pro ExtraLight'; color:#15a362; font-size:large"
                            class="app-page-title mb-3">ទំព័រព័ត៌មានអ្នកផ្គត់ផ្គង់</h1>
                            <div class="mb-2">
                            <button class="btn btn-sm btn-success rounded-5">
                            <a style="color: white; text-decoration:none;" href="supplier_create.php" title="Add New">
                            ...
                            <div class="app-card-body">
                                <div class="table-responsive">
                                    <table class="table app-table-hover mb-0 text-left">
                                        ...
                                        if(isset($_GET['seach'])){
                                            if($_GET['search_str']==''){
                                                $sql_select = $con->prepare("SELECT * FROM tbl_suppliers ORDER BY suppliers_id ASC LIMIT $start_from, $limit");
                                            }else{
                                                $search_str = $_GET['search_str'];
                                                $sql_select = $con->prepare("SELECT * FROM tbl_suppliers WHERE suppliers_name
                                                LIKE '%".$_GET['search_str']."%' ORDER BY suppliers_id ASC LIMIT $start_from, $limit");
                                            }
                                        }else{
                                            $sql_select = $con->prepare("SELECT * FROM tbl_suppliers ORDER BY suppliers_id ASC LIMIT $start_from, $limit");
                                        }
                                        ...
                                        while ($row = $sql_select->fetch(PDO::FETCH_OBJ)) :
                                            $i++;
                                            ?>
                                            <tr id="sup<?= $row->suppliers_id ?>">
                                                <td class="text-center"><?= $i ?></td>
                                                <td><?= $row->suppliers_name ?></td>
                                                <td><?= $row->suppliers_gender ?></td>
                                                <td><?= $row->suppliers_phone ?></td>
                                                <td><?= $row->suppliers_email ?></td>
                                                <td><?= $row->suppliers_address ?></td>
                                                <td class="td-actions text-center">
                                                    <a href="supplier_edit.php?id=<?= $row->suppliers_id ?>" class="p-2" title="Edit">
                                                    ...
                                                    <a href="#" class="text-danger" onclick="delSup(<?= $row->suppliers_id ?>)" class="p-2" title="Remove Supplier">
                                                            <svg width="20" height="20" fill="currentColor" 
                                ...
                                <?php
                                $query = $con->query("SELECT * FROM tbl_suppliers");
                                ...
                                <nav class="app-pagination mt-4">
                                    <ul class="pagination justify-content-center">
                                        <li class="page-item <?= $page == 1 ? 'disabled' : '' ?> ">
                                            <a class="page-link" href="suppliers.php?page=<?= $page - 1 ?>"
                                               tabindex="-1"
                                               aria-disabled="true">Previous</a>
                                        </li>
                                        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                            <li class="page-item active"><a class="page-link"
                                                                            href="suppliers.php?page=<?= $i ?>"><?= $i ?></a>
                                            </li>
                                        <?php endfor; ?>
                                        <li class="page-item <?= $page == $total_pages ? 'disabled' : '' ?> ">
                                            <a class="page-link" href="suppliers.php?page=<?= $page + 1 ?>">Next</a>
                                        </li>
                                    </ul>
                                </nav><!--//app-pagination-->
                                <br>
...
<script type="text/javascript">
    function delSup(id){
        swal.fire({
            title: "Are you sure?",
            text: "Do you really want to remove this Supplier?",
            icon: "warning",
            showDenyButton: true,
